...
public class ToolTip : SingletonMono<ToolTip>
{
	...
	private void Update()
	{
		...
		Vector2 pos;
		RectTransformUtility.ScreenPointToLocalPointInRectangle(CanvasManager.Instance.mainCanvas.transform as RectTransform, Input.mousePosition, CanvasManager.Instance.mainCanvas.worldCamera, out pos);
		//Offsets
		pos.Set(pos.x + (GetComponent<RectTransform>().rect.width * 0.55f), pos.y - (GetComponent<RectTransform>().rect.height * 0.55f));

		//Set the tooltip position
		transform.position = CanvasManager.Instance.mainCanvas.transform.TransformPoint(pos);
	}
